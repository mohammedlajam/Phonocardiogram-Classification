...
def _predict_evaluate_model(x_test_folds, y_test_folds):
    with mlflow.start_run():
        ...
        inception_resnet_models = []
        for fold in range(6):
            loaded_model = load_model(f'/{c.REPO_PATH}/data/models/cv_inception_resnet/cv_inception_resnet_model_{fold}/model.h5')
            inception_resnet_models.append(loaded_model)
        ...
        for fold in range(len(inception_resnet_models)):
            ...
            y_prob = ModelEvaluator.calculate_probabilities(model=inception_resnet_models[fold], x_test=x_test)
        ...
        mlflow.log_param('model', 'cv_inception_resnet')
        ...
        for key, value in evaluation_matrices_folds.items():
            with open(f'cv_inception_resnet_{key}.pkl', 'wb') as f:
                pickle.dump(value, f)
            mlflow.log_artifact(f'cv_inception_resnet_{key}.pkl', artifact_path='evaluation_matrices_folds')
            os.remove(f'cv_inception_resnet_{key}.pkl')
