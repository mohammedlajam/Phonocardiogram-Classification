...
Phase 6: Model Predictions (Computer Vision - Pre-trained Model: "ResNet-50"):
/data/models/tabular_svm
- In this python file, the tabular data is imported from the '/data/models/cv_resnet50/' directory.
...
def _predict_evaluate_model(x_test_folds, y_test_folds):
    with mlflow.start_run():
        ...
        resnet50_models = []
        for fold in range(6):
            loaded_model = load_model(f'/{c.REPO_PATH}/data/models/cv_resnet50/cv_resnet50_model_{fold}/model.h5')
            resnet50_models.append(loaded_model)
        ...
        for fold in range(len(resnet50_models)):
            ...
            y_prob = ModelEvaluator.calculate_probabilities(model=resnet50_models[fold], x_test=x_test)
        ...
        mlflow.log_param('model', 'cv_resnet50')
