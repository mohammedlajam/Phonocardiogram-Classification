...
Phase 6: Model Predictions (Tabular Data - SVM):
/data/models/tabular_svm
- In this python file, the tabular data is imported from the '/data/models/tabular_svm/' directory.
...
def _predict_evaluate_model(x_test_folds, y_test_folds):
    with mlflow.start_run():
        ...
        svm_models = []
        for fold in range(6):
            loaded_model = pickle.load(open(f'/{c.REPO_PATH}/data/models/tabular_svm'
                                            f'/tabular_svm_model_{fold}/model.pkl', 'rb'))
        ...
        cm_folds = []
        test_duration_folds = []
        for fold in range(len(svm_models)):
            ...
            y_prob = ModelEvaluator.calculate_probabilities(model=svm_models[fold], x_test=x_test)
        ...
        mlflow.log_param('model', 'tabular_svm')
