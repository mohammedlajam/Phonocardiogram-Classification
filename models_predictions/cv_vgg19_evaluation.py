...
Phase 6: Model Predictions (Computer Vision - Pre-trained Model: "VGG19"):
/data/models/tabular_svm
- In this python file, the tabular data is imported from the '/data/models/cv_vgg19/' directory.
...
def _predict_evaluate_model(x_test_folds, y_test_folds):
    with mlflow.start_run():
        ...
        vgg19_models = []
        for fold in range(6):
            loaded_model = load_model(f'/{c.REPO_PATH}/data/models/cv_vgg19/cv_vgg19_model_{fold}/model.h5')
            vgg19_models.append(loaded_model)
        ...
        for fold in range(len(vgg19_models)):
            ...
            y_prob = ModelEvaluator.calculate_probabilities(model=vgg19_models[fold], x_test=x_test)
        ...
        mlflow.log_param('model', 'cv_vgg19')
