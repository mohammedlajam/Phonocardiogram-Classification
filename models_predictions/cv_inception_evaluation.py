...
Phase 6: Model Predictions (Computer Vision - Pre-trained Model: "Inception"):
/data/models/tabular_svm
- In this python file, the tabular data is imported from the '/data/models/cv_inception/' directory.
...
def _predict_evaluate_model(x_test_folds, y_test_folds):
    with mlflow.start_run():
        ...
        inception_models = []
        for fold in range(6):
            loaded_model = load_model(f'/{c.REPO_PATH}/data/models/cv_inception/cv_inception_model_{fold}/model.h5')
            inception_models.append(loaded_model)
        ...
        for fold in range(len(inception_models)):
            ...
            y_prob = ModelEvaluator.calculate_probabilities(model=inception_models[fold], x_test=x_test)
        ...
        mlflow.log_param('model', 'cv_inception')
