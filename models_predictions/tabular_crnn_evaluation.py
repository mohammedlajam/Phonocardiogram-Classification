...
Phase 6: Model Predictions (Tabular Data - C-RNN):
/data/models/tabular_svm
- In this python file, the tabular data is imported from the '/data/models/tabular_crnn/' directory.
...
def _predict_evaluate_model(x_test_folds, y_test_folds):
    with mlflow.start_run():
        ...
        crnn_models = []
        for fold in range(6):
            loaded_model = load_model(f'/{c.REPO_PATH}/data/models/tabular_crnn/tabular_crnn_model_{fold}/model.h5')
            crnn_models.append(loaded_model)

        # 2. Predictions:
        evaluation_matrices_folds = []
        cm_folds = []
        test_duration_folds = []
        for fold in range(len(crnn_models)):
            ...
            y_prob = ModelEvaluator.calculate_probabilities(model=crnn_models[fold], x_test=x_test)
        ...
        mlflow.log_param('model', 'tabular_crnn')
