...
Phase 6: Model Predictions (Tabular Data - TabNet):
/data/models/tabular_svm
- In this python file, the tabular data is imported from the '/data/models/tabular_tabnet/' directory.
...
def _predict_evaluate_model(x_test_folds, y_test_folds):
    with mlflow.start_run():
        ...
        tabnet_models = []
        for fold in range(6):
            loaded_model = torch.load(f'/{c.REPO_PATH}/data/models/tabular_tabnet/tabular_tabnet_model_{fold}/')
            tabnet_models.append(loaded_model)

        # 2. Predictions:
        evaluation_matrices_folds = []
        cm_folds = []
        test_duration_folds = []
        for fold in range(len(tabnet_models)):
            ...
            y_prob = ModelEvaluator.calculate_probabilities(model=tabnet_models[fold], x_test=x_test)
        ...
        mlflow.log_param('model', 'tabular_cnn')
