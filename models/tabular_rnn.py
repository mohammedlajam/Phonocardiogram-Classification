...
Phase 5: Classification (Tabular Data - RNN-LSTM):
...
def _tune_hyper_parameters(x_train_folds, x_test_folds, y_train_folds, y_test_folds):
    ...
    for fold in range(len(X_TRAIN_FOLDS)):
        ...
        # Reshaping the x_train and x_val for LSTM:
        x_train = np.reshape(x_train, (x_train.shape[0], x_train.shape[1], 1))
        x_val = np.reshape(x_val, (x_val.shape[0], x_val.shape[1], 1))

        input_shape = x_train.shape[1:]

        best_hp, best_validation = HyperParametersTuner.find_best_tabular_lstm_hp(x_train=x_train,
                                                                                  x_val=x_val,
                                                                                  y_train=y_train,
                                                                                  y_val=y_val,
                                                                                  input_shape=input_shape,
                                                                                  max_trials=c.TB_LSTM_HP_MAX_TRIALS,
                                                                                  epochs=c.TB_LSTM_HP_EPOCHS,
                                                                                  directory=f'/{c.REPO_PATH}/models/hp/tabular_lstm/fold_{fold}/')
    ...
    with mlflow.start_run():
        ...
        for fold in range(len(x_train_folds)):
            ...
            lstm_model, history = ModelBuilder.build_fit_tabular_lstm(x_train=x_train,
                                                                      ...
                                                                      loss=c.TB_LSTM_LOSS,
                                                                      patience=c.TB_LSTM_PATIENCE,
                                                                      epochs=c.TB_LSTM_EPOCHS,
                                                                      batch_size=c.TB_LSTM_BATCH_SIZE)

            # Saving the Model
            model_name = f"tabular_lstm_model_{fold}"
            mlflow.sklearn.log_model(lstm_model, model_name)
            ...
            y_prob = ModelEvaluator.calculate_probabilities(model=lstm_model, x_test=x_test)
        ...
        with open('tabular_lstm_fpr_folds.pkl', 'wb') as f:
            pickle.dump(fpr_folds, f)
        mlflow.log_artifact('tabular_lstm_fpr_folds.pkl', artifact_path='fpr_tpr')
        os.remove('tabular_lstm_fpr_folds.pkl')

        with open('tabular_lstm_tpr_folds.pkl', 'wb') as f:
            pickle.dump(tpr_folds, f)
        mlflow.log_artifact('tabular_lstm_tpr_folds.pkl', artifact_path='fpr_tpr')
        os.remove('tabular_lstm_tpr_folds.pkl')

        # Saving the Model's Summary:
        artifact_path = "tabular_lstm_summary.txt"
        with open(artifact_path, "w") as f:
            lstm_model.summary(print_fn=lambda x: f.write(x + "\n"))
        mlflow.log_artifact(artifact_path, "tabular_lstm_summary.txt")
    ...
    with mlflow.start_run():
        ...
        for fold in range(len(x_train_folds)):
            ...
            lstm_model, history = ModelBuilder.build_fit_tabular_lstm(x_train=x_train,
                                                                      ...
                                                                      y_val=y_val,
                                                                      input_shape=input_shape,
                                                                      lstm_1=c.TB_LSTM_LSTM_1,
                                                                      lstm_2=c.TB_LSTM_LSTM_2,
                                                                      lstm_1_l2=c.TB_LSTM_1_L2,
                                                                      lstm_2_l2=c.TB_LSTM_2_L2,
                                                                      dropout_rate_1=c.TB_LSTM_DROPOUT_RATE_1,
                                                                      dropout_rate_2=c.TB_LSTM_DROPOUT_RATE_2,
                                                                      learning_rate=c.TB_LSTM_LEARNING_RATE,
                                                                      loss=c.TB_LSTM_LOSS,
                                                                      patience=c.TB_LSTM_PATIENCE,
                                                                      epochs=c.TB_LSTM_EPOCHS,
                                                                      batch_size=c.TB_LSTM_BATCH_SIZE)
...
if __name__ == "__main__":
    ...
    if c.TB_LSTM_AUTO_HP:
        ...
        # Building, Fitting and Evaluating LSTM Model based on best Automatic Hyper-Parameters:
        _run_evaluate_lstm_automatic_hp(x_train_folds=X_TRAIN_FOLDS,
                                        x_test_folds=X_TEST_FOLDS,
                                        y_train_folds=Y_TRAIN_FOLDS,
                                        y_test_folds=Y_TEST_FOLDS,
                                        best_hp_folds=BEST_HP_FOLDS)
    else:
        # Building, Fitting and Evaluating LSTM Model based on best Manual Hyper-Parameters:
        _run_evaluate_lstm_manual_hp(x_train_folds=X_TRAIN_FOLDS,
                                     x_test_folds=X_TEST_FOLDS,
                                     y_train_folds=Y_TRAIN_FOLDS,
                                     y_test_folds=Y_TEST_FOLDS)
