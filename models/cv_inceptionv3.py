...
def _run_evaluate_inception(x_train, x_test, y_train, y_test):
    ...
    with mlflow.start_run():
        ...
        inception_model.save('cv_inception.h5')
        ...
        with open('cv_inception_fpr.pkl', 'wb') as f:
            pickle.dump(fpr, f)
        mlflow.log_artifact('cv_inception_fpr.pkl', artifact_path='fpr_tpr')
        os.remove('cv_inception_fpr.pkl')

        with open('cv_inception_tpr.pkl', 'wb') as f:
            pickle.dump(tpr, f)
        mlflow.log_artifact('cv_inception_tpr.pkl', artifact_path='fpr_tpr')
        os.remove('cv_inception_tpr.pkl')
...
if __name__ == "__main__":
    ...
    _run_evaluate_inception(x_train=X_TRAIN,
                            x_test=X_TEST,
                            y_train=Y_TRAIN,
                            y_test=Y_TEST)