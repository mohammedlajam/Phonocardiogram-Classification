...
Phase 5: Classification (Tabular Data - MLP):
...
def _tune_hyper_parameters(x_train_folds, x_test_folds, y_train_folds, y_test_folds):
    ...
    for fold in range(len(X_TRAIN_FOLDS)):
        ...
        best_hp, best_validation = HyperParametersTuner.find_best_tabular_mlp_hp(x_train=x_train,
                                                                                 x_val=x_val,
                                                                                 y_train=y_train,
                                                                                 y_val=y_val,
                                                                                 input_shape=input_shape,
                                                                                 max_trials=c.TB_MLP_HP_MAX_TRIALS,
                                                                                 epochs=c.TB_MLP_HP_EPOCHS,
                                                                                 directory=f'/{c.REPO_PATH}/models/hp/tabular_mlp/fold_{fold}/')
...
def _run_evaluate_mlp_automatic_hp(x_train_folds, x_test_folds, y_train_folds, y_test_folds, best_hp_folds):
    ...
    with mlflow.start_run():
        ...
        for fold in range(len(x_train_folds)):
            ...
            mlp_model, history = ModelBuilder.build_fit_tabular_mlp(x_train=x_train,
                                                                    x_val=x_val,
                                                                    y_train=y_train,
                                                                    y_val=y_val,
                                                                    input_shape=input_shape,
                                                                    ...
                                                                    loss=c.TB_MLP_LOSS,
                                                                    patience=c.TB_MLP_PATIENCE,
                                                                    epochs=c.TB_MLP_EPOCHS,
                                                                    batch_size=c.TB_MLP_BATCH_SIZE)

            # Saving the Model:
            model_name = f"tabular_mlp_model_{fold}"
            mlflow.sklearn.log_model(mlp_model, model_name)
            ...
            y_prob = ModelEvaluator.calculate_probabilities(model=mlp_model, x_test=x_test)
        ...
        mlflow.log_param('patience', c.TB_MLP_PATIENCE)
        mlflow.log_param('epochs', c.TB_MLP_EPOCHS)
        mlflow.log_param('batch_size', c.TB_MLP_BATCH_SIZE)
        ...
        for key, value in evaluation_matrices_folds.items():
            with open(f'tabular_mlp_{key}.pkl', 'wb') as f:
                pickle.dump(value, f)
            mlflow.log_artifact(f'tabular_mlp_{key}.pkl', artifact_path='evaluation_matrices_folds')
            os.remove(f'tabular_mlp_{key}.pkl')

        # Saving the Model's Summary:
        artifact_path = "tabular_mlp_summary.txt"
        with open(artifact_path, "w") as f:
            mlp_model.summary(print_fn=lambda x: f.write(x + "\n"))
        mlflow.log_artifact(artifact_path, "tabular_mlp_summary.txt")
    ...
    with mlflow.start_run():
        ...
        for fold in range(len(x_train_folds)):
            ...
            mlp_model, history = ModelBuilder.build_fit_tabular_mlp(x_train=x_train,
                                                                    ...
                                                                    y_val=y_val,
                                                                    input_shape=input_shape,
                                                                    dense_1=c.TB_MLP_DENSE_1,
                                                                    dense_2=c.TB_MLP_DENSE_2,
                                                                    dense_1_l2=c.TB_MLP_DENSE_1_L2,
                                                                    dense_2_l2=c.TB_MLP_DENSE_2_L2,
                                                                    dropout_rate_1=c.TB_MLP_DROPOUT_RATE_1,
                                                                    dropout_rate_2=c.TB_MLP_DROPOUT_RATE_2,
                                                                    learning_rate=c.TB_MLP_LEARNING_RATE,
                                                                    loss=c.TB_MLP_LOSS,
                                                                    patience=c.TB_MLP_PATIENCE,
                                                                    epochs=c.TB_MLP_EPOCHS,
                                                                    batch_size=c.TB_MLP_BATCH_SIZE)
            ...
            mlp_model.save(f'/{c.REPO_PATH}/data/models/tabular_mlp/tabular_mlp_model_{fold}/model.h5')
            ...
            ax.legend()
            mlflow.log_figure(fig, f'Accuracy_vs_Epochs (Fold_{fold + 1}).png')  # Log the plot in mlflow
            plt.savefig(f'/{c.REPO_PATH}/data/models/tabular_mlp/Accuracy_vs_Epochs_{fold+1}')  # Save the plot
        ...
        # 5. logging all Artifacts, Parameters and Matrices into mlflow:
        # Log the Hyper-Parameters:
        mlflow.log_param('dense_1', c.TB_MLP_DENSE_1)
        mlflow.log_param('dense_2', c.TB_MLP_DENSE_2)
        mlflow.log_param('dense_1_l2', c.TB_MLP_DENSE_1_L2)
        mlflow.log_param('dense_2_l2', c.TB_MLP_DENSE_2_L2)
        mlflow.log_param('dropout_rate_1', c.TB_MLP_DROPOUT_RATE_1)
        mlflow.log_param('dropout_rate_2', c.TB_MLP_DROPOUT_RATE_2)
        mlflow.log_param('learning_rate', c.TB_MLP_LEARNING_RATE)
        mlflow.log_param('patience', c.TB_MLP_PATIENCE)
        mlflow.log_param('epochs', c.TB_MLP_EPOCHS)
        mlflow.log_param('batch_size', c.TB_MLP_BATCH_SIZE)
        ...
        ax.set_title("Confusion Matrix")
        mlflow.log_figure(fig, "confusion_matrix.png")  # Log the plot in mlflow
        plt.savefig(f'/{c.REPO_PATH}/data/models/tabular_mlp/confusion_matrix.png')  # Save the plot
...
if __name__ == "__main__":
    ...
    if c.TB_MLP_AUTO_HP:
        ...
        # Building, Fitting and Evaluating MLP Model based on best Automatic Hyper-Parameters:
        _run_evaluate_mlp_automatic_hp(x_train_folds=X_TRAIN_FOLDS,
                                       x_test_folds=X_TEST_FOLDS,
                                       y_train_folds=Y_TRAIN_FOLDS,
                                       y_test_folds=Y_TEST_FOLDS,
                                       best_hp_folds=BEST_HP_FOLDS)
    else:
        # Building, Fitting and Evaluating MLP Model based on best Manual Hyper-Parameters:
        _run_evaluate_mlp_manual_hp(x_train_folds=X_TRAIN_FOLDS,
                                    x_test_folds=X_TEST_FOLDS,
                                    y_train_folds=Y_TRAIN_FOLDS,
                                    y_test_folds=Y_TEST_FOLDS)
