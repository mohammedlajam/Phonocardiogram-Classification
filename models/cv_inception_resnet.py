...
Phase 5: Classification (Computer Vision - Pre-trained Model: "Inception-ResNet"):
- In this python file, the tabular data is imported from the '/data/processed_data/images/spectrogram/kfold_cv/rgb/'
directory.
/data/processed_data/images/spectrogram/kfold_cv/rgb/
Objective:
...
def _generate_val_set(x_train, x_test, y_train, y_test):
    """Function to prepare the data for InceptionResNetV2."""
...
def _run_evaluate_inceptionresnet(x_train_folds, x_test_folds, y_train_folds, y_test_folds):
    ...
    with mlflow.start_run():
        ...
        for fold in range(len(x_train_folds)):
            ...
            train_dataset = tf.data.Dataset.from_tensor_slices((x_train, y_train)).batch(c.CV_INCEPTIONRESNET_BATCH_SIZE)
            val_dataset = tf.data.Dataset.from_tensor_slices((x_val, y_val)).batch(c.CV_INCEPTIONRESNET_BATCH_SIZE)
            test_dataset = tf.data.Dataset.from_tensor_slices((x_test, y_test)).batch(c.CV_INCEPTIONRESNET_BATCH_SIZE)

            input_shape = x_train[0].shape

            # 1. Building and fitting MLP Model:
            inceptionresnet_model, history = PretrainedModel.build_fit_inceptionresnet(train_dataset=train_dataset,
                                                                                       val_dataset=val_dataset,
                                                                                       input_shape=input_shape,
                                                                                       include_top=c.CV_INCEPTIONRESNET_INCLUDE_TOP,
                                                                                       inceptionresnet_weights=c.CV_INCEPTIONRESNET_WEIGHTS,
                                                                                       trainable=c.CV_INCEPTIONRESNET_TRAINABLE,
                                                                                       dense_1=c.CV_INCEPTIONRESNET_DENSE_1,
                                                                                       dense_2=c.CV_INCEPTIONRESNET_DENSE_2,
                                                                                       dense_1_l2=c.CV_INCEPTIONRESNET_DENSE_1_L2,
                                                                                       dense_2_l2=c.CV_INCEPTIONRESNET_DENSE_2_L2,
                                                                                       dropout_rate_1=c.CV_INCEPTIONRESNET_DROPOUT_RATE_1,
                                                                                       dropout_rate_2=c.CV_INCEPTIONRESNET_DROPOUT_RATE_2,
                                                                                       learning_rate=c.CV_INCEPTIONRESNET_LEARNING_RATE,
                                                                                       loss=c.CV_INCEPTIONRESNET_LOSS,
                                                                                       patience=c.CV_INCEPTIONRESNET_PATIENCE,
                                                                                       epochs=c.CV_INCEPTIONRESNET_EPOCHS)

            # Saving the Model
            model_name = f"cv_inception_resnet_{fold}"
            mlflow.sklearn.log_model(inceptionresnet_model, model_name)
            # Saving the model in data directory:
            inceptionresnet_model.save(f'/{c.REPO_PATH}/data/models/cv_inception_resnet/cv_inception_resnet_model_{fold}/model.h5')
            ...
            plt.savefig(f'/{c.REPO_PATH}/data/models/cv_inception_resnet/Accuracy_vs_Epochs_{fold + 1}')  # Save the plot

            # 2. Predictions:
            # Calculating the Probabilities:
            start_time = time.time()  # Measuring the start_time of the predictions
            y_prob = ModelEvaluator.calculate_probabilities(model=inceptionresnet_model, x_test=test_dataset)
        ...
        mlflow.log_param('include_top', c.CV_INCEPTIONRESNET_INCLUDE_TOP)
        mlflow.log_param('weights', c.CV_INCEPTIONRESNET_WEIGHTS)
        mlflow.log_param('trainable', c.CV_INCEPTIONRESNET_TRAINABLE)
        mlflow.log_param('dense_1', c.CV_INCEPTIONRESNET_DENSE_1)
        mlflow.log_param('dense_2', c.CV_INCEPTIONRESNET_DENSE_2)
        mlflow.log_param('dense_1_l2', c.CV_INCEPTIONRESNET_DENSE_1_L2)
        mlflow.log_param('dense_2_l2', c.CV_INCEPTIONRESNET_DENSE_2_L2)
        mlflow.log_param('dropout_rate_1', c.CV_INCEPTIONRESNET_DROPOUT_RATE_1)
        mlflow.log_param('dropout_rate_2', c.CV_INCEPTIONRESNET_DROPOUT_RATE_2)
        mlflow.log_param('learning_rate', c.CV_INCEPTIONRESNET_LEARNING_RATE)
        mlflow.log_param('patience', c.CV_INCEPTIONRESNET_PATIENCE)
        mlflow.log_param('epochs', c.CV_INCEPTIONRESNET_EPOCHS)
        mlflow.log_param('batch_size', c.CV_INCEPTIONRESNET_BATCH_SIZE)
        ...
        for key, value in evaluation_matrices_folds.items():
            with open(f'cv_inceptionresnet_{key}.pkl', 'wb') as f:
                pickle.dump(value, f)
            mlflow.log_artifact(f'cv_inceptionresnet_{key}.pkl', artifact_path='evaluation_matrices_folds')
            os.remove(f'cv_inceptionresnet_{key}.pkl')

        # Saving the Model's Summary:
        artifact_path = "cv_inceptionresnet_summary.txt"
        with open(artifact_path, "w") as f:
            inceptionresnet_model.summary(print_fn=lambda x: f.write(x + "\n"))
        mlflow.log_artifact(artifact_path, "cv_inceptionresnet_summary.txt")
        ...
        plt.savefig(f'/{c.REPO_PATH}/data/models/cv_inception_resnet/confusion_matrix.png')  # Save the plot
...
if __name__ == "__main__":
    ...
    # Building, Fitting and Evaluating Inception_ResNet:
    _run_evaluate_inceptionresnet(x_train_folds=X_TRAIN_FOLDS,
